...
import { TableProps, ColumnProps, SorterResult, TableRowSelection } from 'antd/es/table';
...
class StandardTable extends Component<StandardTableProps<TableListItem>, StandardTableState> {
  ...
  handleRowSelectChange = (selectedRowKeys: string[] | number[], selectedRows: TableListItem[]) => {
    ...
    needTotalList = needTotalList.map(item => ({
      ...item,
      total: selectedRows.reduce((sum, val) => (sum + parseFloat(val[item.dataIndex? item.dataIndex : ''])), 0),
    }));
